const STORE='mt-risk-sim-v16';let DATA,state,undoStack=[];let sb=null;
...
async function boot(){DATA=await fetch('game-data.json?v=16').then(r=>r.json());state=load()||fresh();render();await syncAllToSupabase();}
...
function render(){
  ...
  state.mts.forEach(mt=>{
    ...
    <div class="choice-history"><h4>Keuzes deze sessie</h4><div class="choice-counts">${['A','B','C','D'].map(ch=>`<div class="choice-count"><span>${ch}</span><b>${counts[ch]}</b></div>`).join('')}</div></div>
