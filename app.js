const STORE='mt-risk-sim-v19';let DATA,state,undoStack=[];let sb=null;
...
async function boot(){DATA=await fetch('game-data.json?v=19').then(r=>r.json());state=load()||fresh();render();await syncAllToSupabase();}
