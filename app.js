const STORE='mt-risk-sim-v25-3';let DATA,state,undoStack=[],PROFILE_DATA=null;let sb=null;
...
async function boot(){DATA=await fetch('game-data.json?v=25.3').then(r=>r.json());try{PROFILE_DATA=await fetch('mt-profiles.json?v=25.3').then(r=>r.ok?r.json():null)}catch(e){console.warn('Profieldata kon niet worden geladen',e);PROFILE_DATA=null;}state=load()||fresh();
