const STORE='mt-risk-sim-v20';let DATA,state,undoStack=[];let sb=null;
...
async function boot(){DATA=await fetch('game-data.json?v=20').then(r=>r.json());state=load()||fresh();render();await syncAllToSupabase();}
